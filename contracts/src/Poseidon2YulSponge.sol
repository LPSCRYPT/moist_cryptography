...
/// @notice Poseidon2 sponge over BN254 Fr, hand-Yul'd for gas-efficient
///         on-chain binding checks. Matches the shape of
///         `FeatureToken._poseidon2Sponge249` and the Noir mint circuit's
///         `poseidon2_sponge_X`:
///
///             state = [0, 0, 0, 0]                    // no length-prefix IV
///             for each 3-field block in input:
///                 state[0] += block[0]
///                 state[1] += block[1]
///                 state[2] += block[2]
///                 state = perm(state)
///             state[0] += 1                            // padding sentinel
///             state = perm(state)
///             return state[0]
///
///         Calldata: N 32-byte big-endian Field elements concatenated, where N
///         is a multiple of 3. For sponge_249, calldatasize() must equal 7968.
///         Returns: 32 bytes = state[0] reduced mod PRIME.
///
/// @dev Permutation body extracted verbatim from
///      `poseidon2-evm/Poseidon2Yul.sol` (one fully-unrolled BN254-Fr round
///      stack with every constant inlined as an immediate). The sponge wrapper
///      adds the absorb loop, sentinel-pad and final-perm. After perm, all
///      four lanes are reduced mod PRIME so the next iteration's absorb is
///      safe under the hand-tuned `add`/`addmod` discipline of the original.
contract Poseidon2YulSponge {
    /// @dev Stateless. Call via STATICCALL with the input as calldata.
    fallback() external {
        assembly {
            let PRIME := 0x30644e72e131a029b85045b68181585d2833e84879b9709143e1f593f0000001
            let cs := calldatasize()
            // Reject non-multiple-of-3 inputs; tail-padding stays a
            // circuit-side responsibility for our use cases (sponge_249,
            // sponge_42 etc. are all multiples of 3 by construction).
            if mod(cs, 96) { revert(0, 0) }

            let blocks := div(cs, 96)
            let s0 := 0
            let s1 := 0
            let s2 := 0
            let s3 := 0
            ...
            // ----------------------------------------------------------------
            // Yul function: one full Poseidon2 BN254-Fr permutation, unrolled.
            // ----------------------------------------------------------------
            function permute(in0, in1, in2, in3) -> out0, out1, out2, out3 {
                let P_FN := 0x30644e72e131a029b85045b68181585d2833e84879b9709143e1f593f0000001
                let state0 := in0
                let state1 := in1
                let state2 := in2
                let state3 := in3
...

                {
                    ...
                    t3 := add(t3, t0)
                    let t4 := add(t1, t1)
                    t4 := addmod(t4, t4, P_FN)
                    t4 := add(t4, t3)
                    let t5 := add(t0, t0)
                    t5 := addmod(t5, t5, P_FN)
                    t5 := add(t5, t2)
                    let t6 := addmod(t3, t5, P_FN)
                    let t7 := addmod(t2, t4, P_FN)
                    state0 := t6
                    state1 := t5
                    state2 := t7
                    state3 := t4
                    state1 := mod(state1, P_FN)
                    state3 := mod(state3, P_FN)
                }
...

                {
                    // single_box
                    let intr := state0
                    state0 := mulmod(intr, intr, P_FN)
                    state0 := mulmod(state0, state0, P_FN)
                    state0 := mulmod(state0, intr, P_FN)
                }

                {
                    // single_box
                    let intr := state1
                    state1 := mulmod(intr, intr, P_FN)
                    state1 := mulmod(state1, state1, P_FN)
                    state1 := mulmod(state1, intr, P_FN)
                }

                {
                    // single_box
                    let intr := state2
                    state2 := mulmod(intr, intr, P_FN)
                    state2 := mulmod(state2, state2, P_FN)
                    state2 := mulmod(state2, intr, P_FN)
                }

                {
                    // single_box
                    let intr := state3
                    state3 := mulmod(intr, intr, P_FN)
                    state3 := mulmod(state3, state3, P_FN)
                    state3 := mulmod(state3, intr, P_FN)
                }
                ...
                // Reduce all four lanes so the next absorb's addmod operates
                // on canonical Fr elements (< P_FN).
                out0 := mod(state0, P_FN)
                out1 := mod(state1, P_FN)
                out2 := mod(state2, P_FN)
                out3 := mod(state3, P_FN)
            }
        }
    }
}
