...
/// @notice Poseidon2 sponge over BN254 Fr for **exactly 16 field inputs**.
/// Hand-Yul'd variant of Poseidon2YulSponge with a different absorption
/// pattern matching the `sponge_16` shape used by transfer_shadow_v2 and
/// solve_shadow_v2 circuits, plus zindex_commit:
///
///   state = [0, 0, 0, 0]
///   for b in 0..5: absorb(e[3b], e[3b+1], e[3b+2]); permute()  // 5 full rate-3 blocks
///   absorb(e[15], 0, 0); permute()                              // partial 6th block
///   state[0] += 1; permute()                                    // sentinel
///   return state[0]
///
/// Calldata: exactly 16 × 32 = 512 bytes, big-endian fields.
/// Returns: 32 bytes = state[0] reduced mod PRIME.
///
/// @dev Permutation body identical to Poseidon2YulSponge (verbatim copy).
contract Poseidon2YulSponge16 {
    ...
    fallback() external {
        assembly {
            ...
            let cs := calldatasize()
            // Reject anything except exactly 16 fields (512 bytes).
            if iszero(eq(cs, 512)) { revert(0, 0) }
            ...
            // Partial 6th block: absorb e[15] only into state[0].
