...
/// @notice Poseidon2 BN254-Fr 2-input compression. Calldata = exactly 2
///         Field elements (64 bytes). Returns 32 bytes = state[0] of
///         `poseidon2_permutation([a, b, 0, 0], 4)`, matching the Noir
///         primitive `poseidon2_hash_2` used across our v2 circuits:
///
///             poseidon2_hash_2(a, b) = poseidon2_permutation([a,b,0,0], 4)[0]
///
///         Unlike `Poseidon2YulSponge`, there is NO absorb loop and NO
///         sentinel pad: this is a fixed-input compression, not a sponge.
///         Input lanes are reduced mod PRIME before entering the
///         permutation (so non-canonical Fr inputs are silently
///         normalized rather than rejected; canonical inputs round-trip).
///
/// @dev Permutation body identical to Poseidon2YulSponge (verbatim copy).
///      Stateless. Call via STATICCALL with 64 bytes of input.
contract Poseidon2YulHash2 {
    fallback() external {
        assembly {
            let PRIME := 0x30644e72e131a029b85045b68181585d2833e84879b9709143e1f593f0000001
            let cs := calldatasize()
            // Reject anything except exactly 2 Field elements.
            if iszero(eq(cs, 64)) { revert(0, 0) }

            // Lane init: state = [a, b, 0, 0], inputs reduced mod PRIME so
            // the permutation operates on canonical Fr elements.
            let s0 := mod(calldataload(0),  PRIME)
            let s1 := mod(calldataload(32), PRIME)
            let s2 := 0
            let s3 := 0
            ...
            // Body copied verbatim from Poseidon2YulSponge.sol.
            // ----------------------------------------------------------------
